...
export class Rack extends React.Component {
  ...
  drawRack(
    ...
    elements.push(
      this.drawSide(rackPxWidth, rackPxOffset, x1, y3, "bottomBar"),
    );
    elements.push(this.drawSide(rackPxOffset, rackPxHeight, x1, y2, "leftBar"));
    elements.push(
      this.drawSide(rackPxOffset, rackPxHeight, x2, y2, "rightBar"),
    );
    elements.push(this.drawLabel(middle, y3, this.props.label, "center", true));

    if (this.props.displayRmu) {
      elements.push(
        this.drawHeightMarkers(inchToRmu, middle, x1, y3, pxToInch),
      );
    }
    ...
    const newChildren = React.Children.map(childElements, (child) => {
      ...
      const currentRmuFrontPositions = childMap.front
        .filter(
          (c) =>
            c.name ===
            `${child.props.label}-${child.props.rmu}-${child.props.facing}`,
        )
        .map((v) => v.position);
  ...
  render() {
    ...
    this.props.children.forEach((child) => {
      if (this.props.facing === "Front" && child.props.facing === "Front") {
        ...
        this.props.facing === "Front" &&
        child.props.facing === "Back"
      ) {
        bottomChildren.push(child);
      } else if (
        this.props.facing === "Back" &&
        child.props.facing === "Back"
      ) {
        topChildren.push(child);
      } else if (
        this.props.facing === "Back" &&
        child.props.facing === "Front"
      ) {
        bottomChildren.push(child);
      }
    });
